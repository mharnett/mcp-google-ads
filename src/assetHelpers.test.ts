...
describe("parseAssetLinkResourceName", () => {
  it("parses a customer_asset resource name", () => {
    const r = parseAssetLinkResourceName("customers/7458517309/customerAssets/310221732713~SITELINK");
    expect("error" in r).toBe(false);
    if ("error" in r) return;
    expect(r.level).toBe("customer_asset");
    expect(r.customer_id).toBe("7458517309");
  });

  it("parses a campaign_asset resource name", () => {
    const r = parseAssetLinkResourceName("customers/7458517309/campaignAssets/22988718227~23328945762~SITELINK");
    expect("error" in r).toBe(false);
    if ("error" in r) return;
  ...
  it("parses an ad_group_asset resource name", () => {
    const r = parseAssetLinkResourceName("customers/7458517309/adGroupAssets/186197514938~200773811124~SITELINK");
    expect("error" in r).toBe(false);
    if ("error" in r) return;
  ...
  it("rejects a bare asset resource name (not a link)", () => {
    const r = parseAssetLinkResourceName("customers/7458517309/assets/310221732713");
...
describe("validateFinalUrls", () => {
  it("accepts a single https URL", () => {
    const r = validateFinalUrls(["https://flow.space/schedule-a-demo"]);
    expect(r.ok).toBe(true);
  });

  it("trims whitespace", () => {
    const r = validateFinalUrls(["  https://flow.space/a  "]);
    if (r.ok) expect(r.urls[0]).toBe("https://flow.space/a");
  });
  ...
  it("rejects URL without scheme", () => {
    expect(validateFinalUrls(["flow.space/x"]).ok).toBe(false);
  });
...
describe("normalizeUpdateAssetUrlsArgs", () => {
  it("normalizes a valid single-update call", () => {
    const r = normalizeUpdateAssetUrlsArgs({
      ...
      updates: [{ asset_id: "23328945732", final_urls: ["https://flow.space/fulfillment/"] }],
    });
    expect("error" in r).toBe(false);
    if ("error" in r) return;
...
describe("normalizePauseAssetLinksArgs", () => {
  it("normalizes valid resource names", () => {
    const r = normalizePauseAssetLinksArgs({
      resource_names: [
        "customers/7458517309/customerAssets/310221732713~SITELINK",
        "customers/7458517309/campaignAssets/22988718227~23328945762~SITELINK",
      ],
    });
    expect("error" in r).toBe(false);
    if ("error" in r) return;
  ...
  it("accepts resource_names passed as a JSON-encoded string", () => {
    const r = normalizePauseAssetLinksArgs({
      resource_names: JSON.stringify(["customers/7458517309/customerAssets/310221732713~SITELINK"]) as any,
    });
    expect("error" in r).toBe(false);
  ...
  it("buckets by level", () => {
    const dr = buildPauseLinksDryRun({
      ...
      resource_names: [
        "customers/7458517309/customerAssets/310221732713~SITELINK",
        "customers/7458517309/campaignAssets/22988718227~23328945762~SITELINK",
        "customers/7458517309/adGroupAssets/186197514938~200773811124~SITELINK",
      ],
    });
