 ...
 * the wrong case. Either way, no user-facing signal that the contract
 * broke.
 *
 * This test parses index.ts as text, extracts every `case "google_ads_..."`
 * string, and asserts the case set equals the declared tool set. When a
 * tool is added/removed, the author must update BOTH sides or the test
 * fails with the offending name.
 *
 * Pattern source: google-ads-recommendations/tests/unit/recTypeExhaustiveness.
 */
...
/**
 * Extract every `case "<name>":` string literal that looks like a tool name.
 * We constrain to the google_ads_ prefix so we don't pick up unrelated
 * switch statements (e.g. channel_type enum branches).
 */
